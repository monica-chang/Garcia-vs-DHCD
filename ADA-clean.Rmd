...
# the hyphens from the pei, parses out the accommodation type, and expresses
...
clean_ada_request <- function(x) {
  x <- x %>%
    ...
    mutate(req_a_anim = as.double(unlist(req_a_anim)), .keep = "unused") %>%
    mutate(req_a_care = as.double(unlist(req_a_care)), .keep = "unused") %>%
    mutate(req_a_creg = as.double(unlist(req_a_creg)), .keep = "unused") %>%
    mutate(req_a_cook = as.double(unlist(req_a_cook)), .keep = "unused") %>%
    mutate(req_a_ffloor = as.double(unlist(req_a_ffloor)), .keep = "unused") %>%
    mutate(req_a_ncarp = as.double(unlist(req_a_ncarp)), .keep = "unused") %>%
    mutate(req_a_pmod = as.double(unlist(req_a_pmod)), .keep = "unused") %>%
    mutate(req_a_scat = as.double(unlist(req_a_scat)), .keep = "unused") %>%
    mutate(req_a_serv = as.double(unlist(req_a_serv)), .keep = "unused") %>%
    mutate(req_a_wchair = as.double(unlist(req_a_wchair)), .keep = "unused") %>%
    mutate(req_a_oth = as.double(unlist(req_a_oth)), .keep = "unused")
...
# Combine the 2018 and 2020 touchpoint data. Error with col types.

ada_request_2018_2020 <- bind_rows(ada_request_2018, ada_request_2020)

# TODO: Parse the request reasons. Add in the other acc type + req reasons.
