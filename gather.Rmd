...
fit_obj_tbl <- as_tibble(fit_obj) %>%
  rename(mu = `(Intercept)`) %>%
  mutate(mu = median(mu)) %>%
  mutate(request = median(request)) %>%
  mutate(reason = median(reason)) %>%
  select(mu, request, reason)

fit_obj_tbl %>% 
  mutate(request = req_scattered_site_placement_unit_co_housing_unit + mu) %>%
  mutate(reason = reason_mental_health + mu) %>%
  select(mu, request, reason) 
