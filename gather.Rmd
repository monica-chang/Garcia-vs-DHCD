...
fit_obj <- stan_glm(days_until_accommodation_met ~ req_scattered,
...
fit_obj_tbl <- as_tibble(fit_obj) %>%
  rename(mu = `(Intercept)`) %>%
  mutate(reason_mental_health = reason_mental_health + mu) %>%
  mutate(req_scattered_site_placement_unit_co_housing_unit = req_scattered_site_placement_unit_co_housing_unit + mu) %>%
  select(-sigma) 

fit_obj_stat <- tibble(mu = median(fit_obj_tbl$mu),
                       reason_mental_health = median(fit_obj_tbl$reason_mental_health),
                       req_scattered_site_placement_unit_co_housing_unit = 
                       median(fit_obj_tbl$req_scattered_site_placement_unit_co_housing_unit))

saveRDS(fit_obj_stat, file = "shiny/data/fit_obj_stat.rds")

fit_obj_tbl %>% 
  pivot_longer(cols = mu:req_scattered_site_placement_unit_co_housing_unit,
               names_to = "parameter",
               values_to = "wait_time") %>%
...
supplemented_interesting_ada_transfers %>%
  ggplot(aes(x = reason_mental_health, y = days_until_accommodation_met, group = reason_mental_health)) + 
    geom_boxplot() +
    ylim(0, 200)
