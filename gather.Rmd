...
col_types <- cols(
  .default = col_character(),
  date_referral_was_accepted_or_rejected = col_datetime(format = "")
)
...
external_transfers <- 
  transform(external_transfers, date_referral = as_datetime(date_referral)) %>%
  mutate(pei = gsub("-", "", pei)) %>%
...
transfers_chars_hotel_t %>%
  ...
  group_modify( ~ {
    .x %>%
      arrange(date_referral) %>%
      mutate(date_acc_met = ifelse(nrow(.x %>% filter(placement_acc_met == 1)) >= 1, 
                                   head(.x %>% filter(placement_acc_met == 1))$date_referral,
                                   NA))
    }) %>% View()
...
#    - Keep the earliest one that meets the need and get rid of the other rows.
# - If request was never met, set a 1-year censorship date for date_acc_met. Keep the earliest row.
