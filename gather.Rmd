...
breakdown_by_time_acc <- all_adas %>%
  ...
  group_by(accommodation_type, year, approved) %>%
  filter(!is.na(year)) %>%
  summarize(total_requests = sum(requests), .groups = "drop") %>%
...
ggplot(breakdown_by_time_acc, aes(x = year, y = percent, color = accommodation_type)) + 
  geom_line() +
  theme_linedraw() +
  scale_color_discrete(name = "Accommodation Type") +
  labs(title = "Change in ADA approval rate by accommodation type (2015-2019)",
       subtitle = "The approval rate for disability accommodation requests significantly decreased from 2016 to 2018.",
       x = "Year",
       y = "Percentage of disability accommodation requests approved")
...
ggplot(breakdown_by_time, aes(x = year, y = percent)) + 
  geom_line() +
  theme_linedraw() +
  ylim(0, 100) +
