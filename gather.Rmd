...
clean_ada_request <- function(x) {
  x <- x %>%
    ...
    rename(pei = participant_enterprise_identifier,
           ...
           # ASK: What is the diff between date received and date decision and date taken? Why does date taken appear after date decision?
           ...
           # ASK: Why are "Yes" "No" "NA" "Partial" and "Interactive Process" options? Only 2027/3571 are "Yes". Should I only analyze these? Do we really trust the DHCD's ADA decisions?
...
# TODO: Find date to accommodation met information.
...
# ASK: 54 are negative # of days... so the date_acc_met is before the date_received? 
#      587 are positive # of days. 558 are approved & positive # of days.

ada_request_2018_2020 %>%
  filter(days_until_accommodation_met >= 0) %>% 
  group_by(days_until_accommodation_met) %>%
  summarize(num_requests_met = n()) %>%
