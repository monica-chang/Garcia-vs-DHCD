...
ada_transfers <- left_join(approved_adas, transfers, by = "pei") %>%
  ...
  mutate(days_until_accommodation_met = as.double(difftime(ymd(date_acc_met),
                                                           ymd(date_received),
                                                           units = "days"))) %>%
...
supplement_date_acc_met <- function(df){
  df %>%
    ...
    # because I have insufficient data to account for placement close to service
    # providers, other, assistance animals, access to full cooking facilities, 
    # change in EA regulation, and assigned caretaker. Only 1282/3143 rows remain.
    ...
    mutate(date_acc_met = ifelse(!is.na(date_acc_met), 
                                 ...
                                 ifelse(placement_acc_met == 1, 
                                        date_referral, 
                                        date_received))) %>%
    mutate(date_acc_met = strptime("1970-01-01", "%Y-%m-%d", tz="GMT") + date_acc_met) %>%
    mutate(date_acc_met = ymd(date_acc_met)) %>%
    mutate(days_until_accommodation_met = as.double(difftime(ymd(date_acc_met),
                                                             ymd(date_received),
                                                             units = "days")))
}

supplement_date_acc_met(transfers_chars_hotel_t)
