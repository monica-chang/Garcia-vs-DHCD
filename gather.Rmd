...
create_kaplan_meier(clean_ada_transfers)
```
...
clean_ada_transfers <- supplement_date_acc_met(transfers_chars_hotel_t)

# TODO: Clean & merge HUD exit data. 

# After we have a Kaplan-Meier plot that is more reliable, we may be able to have 
# a binary question (was this request met within 30 days or not) and identify
# a reasonable cut off date. Percentages are easier to understand.

# Is there demographic information for these individuals? 
...
# outcome - # of days until request is met (wait time) OR whether request was met?
# covariates - reason for request (mental or physical) or type of request (scattered site or not)
...
fit_obj <- stan_glm(days_until_accommodation_met ~ request_reason + accommodation_type
                    data = week_8,
                    refresh = 0,
                    family = gaussian)
