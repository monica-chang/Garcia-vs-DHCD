...
ggplot(breakdown_by_time_acc, aes(x = date, y = total)) + 
  ...
  theme_linedraw() +
  scale_color_discrete(name = "Accommodation Type") +
  labs(title = "Change in total number of requests (2015-2019)",
       x = "Month",
...
accommodation_rate_2 <- left_join(ip_respond, ip_respond_acc, by = c("year", "month", "date")) %>%
  mutate(percent = accommodated/responded * 100) %>%
  select(date, accommodated, responded, percent)

ggplot(accommodation_rate_2, aes(x = date, y = percent)) + 
  geom_point() +
  geom_line() +
  theme_linedraw() +
