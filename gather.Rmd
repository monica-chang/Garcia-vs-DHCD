...
external_transfers <- read_excel("raw_data/combined_Referrals_EA_Placement_Unit.xlsx") %>%
  ...
  arrange(date_ext_referral) 

saveRDS(external_transfers, file = "shiny/data/external_transfers.rds")
...
internal_transfers <- read_excel("raw_data/combined_EA_Providers_Internal_Transfers.xlsx") %>%
  ...
  # I filter out all internal transfer to stabilization units.
  ...
  arrange(date_int_referral) 

saveRDS(internal_transfers, file = "shiny/data/internal_transfers.rds")
...
ext_join <- left_join(approved_adas, external_transfers, by = "pei") %>%
  filter(date_received < date_ext_referral | is.na(date_ext_referral)) 
  
int_join <- left_join(approved_adas, internal_transfers, by = "pei") %>% 
  filter(date_received < date_int_referral | is.na(date_int_referral))
```
