...
clean_ada_request <- function(x) {
  x <- x %>%
    ...
    pivot_longer(cols = c(a, b, c, d, e), 
                 names_to = "letter_id",  
                 values_to = "accommodation_request") %>%
    ...
    mutate(accommodation_request = recode(accommodation_request, 
    ...
    pivot_wider(id_cols = pei:date_acc_met, 
                names_from = accommodation_request, 
                values_from = dummy, 
                values_fill = 0) %>%
    ...
    rename(req_a_scat = scattered_site_placement_unit_co_housing_unit) %>%  
    ...
    select(pei, date_received, date_decision, date_taken, date_acc_met, 
           req_where:how_acc_met, req_a_anim, req_a_care, req_a_creg, req_a_cook, 
           req_a_ffloor, req_a_ncarp, req_a_pmod, req_a_scat, req_a_serv, 
           req_a_wchair, req_a_oth) %>%
    ...
    mutate(reason_for_request = recode(reason_for_request, 
                                      `Developmental Disability` = "Developmental Disability / Behavioral",
    ...
    clean_names() %>%
    rename(reason_mental = mental_emotional_health) %>% 
    rename(reason_emotional = emotional_health) %>%
    rename(reason_physical = physical_health) %>%
    rename(reason_devel_behav = developmental_disability_behavioral) %>%
    ...
    select(pei:date_acc_met, days_until_accommodation_met, req_where:req_a_oth, 
           reason_mental, reason_mental, reason_emotional, reason_physical, 
           reason_devel_behav, reason_other)
...
approved_requests <- ada_request_2018_2020 %>%
  filter(approved == "Yes") %>%
  pivot_longer(cols = req_a_anim:req_a_oth, names_to = "accomodation_type", values_to = "requests") %>%
  group_by(accomodation_type) %>%
  summarize(total_requests = sum(requests), .groups = "drop") %>%
...
ggplot(approved_requests, aes(x = pct_requests, y = fct_reorder(accomodation_type, pct_requests))) +
  geom_col() +
  theme_linedraw() +
  scale_y_discrete(labels = c("Caretaker", "Wheelchair", "Cooking facilities", "Physical modification", "Change in regulation", "Non-carpeted", "First floor/elevator", "Assistance animal", "Other", "Scattered site", "Service providers")) +
...
approved_reasons <- ada_request_2018_2020 %>%
  filter(approved == "Yes") %>%
  pivot_longer(cols = reason_mental:reason_other, names_to = "reason_type", values_to = "requests") %>%
  group_by(reason_type) %>%
  summarize(total_requests = sum(requests), .groups = "drop") %>%
...
ggplot(approved_reasons, aes(x = pct_requests, y = fct_reorder(reason_type, pct_requests))) +
  geom_col() +
  theme_linedraw() +
  scale_y_discrete(labels = c("Other", "Developmental/ \nbehavioral disability", "Emotional health", "Mental health", "Physical health")) +
