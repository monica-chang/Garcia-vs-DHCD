...
# I pivot my data so that each row represents one request i.e. each row is 
# uniquely identified by PEI + date_received.
...
all_requests <- all_adas %>%
                ...
                mutate(percent = total_requests/sum(total_requests))
...
requests <- left_join(all_requests, approved_requests, by = "accommodation_type") %>%
  ...
  mutate(percent_approved = num_approved_requests/num_all_requests) %>%
...
interesting_approved_adas <- approved_adas_include_NA %>% 
  filter(interesting == 1) 
