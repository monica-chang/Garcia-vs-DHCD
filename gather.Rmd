...
random_zips <- rep_sample_n(as.tibble(boston_zips), 1, replace = TRUE, reps = 2108)

approved_adas_dates_fixed <- all_adas %>%
  filter(approved == "Yes") %>%
  select(-approved) %>%
  ...
  mutate(zipcode_of_provider = random_zips$value,
         distance_from_provider_required = 20,
         type_of_provider = NA,
         misc_notes = NA)
...
```{r create time plots for approved requests}
...
# not approved, and the percentage approved for each accommodation type and year.
...
breakdown_by_time_acc <- all_adas %>%
  ...
  filter(accommodation_type == "Scattered Site Placement Unit / Co-housing Unit") %>%
...
# I create a plot showing the change in approval rate for each accommodation
# type over time.
...
ggplot(breakdown_by_time_acc, aes(x = date, y = percent)) + 
  ...
  labs(title = "Change in ADA approval rate for scattered site placement (2015-2019)",
       x = "Month",
       y = "Percentage of requests approved")

ggplot(breakdown_by_time_acc, aes(x = date, y = total)) + 
  geom_point() +
  geom_line() +
  theme_linedraw() +
  scale_color_discrete(name = "Accommodation Type") +
  labs(title = "Change in total number of requests for scattered site placement (2015-2019)",
       x = "Month",
...
ggplot(breakdown_by_time_acc, aes(x = date, y = yes)) + 
  ...
  theme_linedraw() +
  scale_color_discrete(name = "Accommodation Type") +
  labs(title = "Change in number of approved requests for scattered site placement (2015-2019)",
       x = "Month",
       y = "Number of approved requests")

breakdown_by_time <- breakdown_by_time_acc %>%
  group_by(date) %>%
  summarize(total_yes = sum(yes), total_no = sum(no), .groups = "drop") %>%
  mutate(total = total_yes + total_no) %>%
  mutate(percent = total_yes/(total_yes + total_no) * 100) 

# I create a plot showing the change in approval rate over time.

ggplot(breakdown_by_time, aes(x = date, y = percent)) + 
  geom_point() +
  geom_line() +
  theme_linedraw() +
  labs(title = "Change in ADA approval rate (2015-2019)",
       subtitle = "The approval rate for ADA requests significantly decreased after 2017.",
       x = "Month",
       y = "Percentage of requests approved")

# I create a plot showing the change in total number of ADA requests over time.

ggplot(breakdown_by_time, aes(x = date, y = total)) + 
  geom_point() +
  geom_line() +
  theme_linedraw() +
  labs(title = "Change in total number of ADA requests (2015-2019)",
       subtitle = "The number of total ADA requests increased after 2017.",
       x = "Month",
       y = "Total number of ADA requests")

# I create a plot showing the change in total number of approved ADA requests over time.
  
ggplot(breakdown_by_time, aes(x = date, y = total_yes)) + 
  geom_point() +
  geom_line() +
  theme_linedraw() +
  labs(title = "Change in number of approved ADA requests (2015-2019)",
       subtitle = "The number of approved ADA requests has stayed relatively constant over time, \n despite increased number of applicants.",
       x = "Month",
       y = "Number of approved ADA requests")
...
ada_transfers <- left_join(approved_adas_dates_fixed, transfers, by = "pei") %>% 
  ...
  # address the disability request. 2882/5072 rows remain.
...
# unit type accommodations. 2510 observations.
...
transfers_chars_hotel_t <- 
  ...
  mutate(zipcode_of_transfer = paste0(zipcode_of_transfer.x, zipcode_of_transfer.y)) %>%
...
btwn_zips <- transfers_chars_hotel_t %>%
  select(pei, zipcode_of_transfer, zipcode_of_provider) %>%
  mutate(zipcode_of_transfer = str_c(zipcode_of_transfer, ", USA")) %>%
  mutate(zipcode_of_provider = str_c(zipcode_of_provider, ", USA")) %>%
  replace_na(list(zipcode_of_transfer = "98607, USA", zipcode_of_provider = "98607, USA")) %>%
  head(100) %>%
...
# first floor, or wheelchair, I substitute missing date_acc_met values with 
# transfer information and assume that hotel transfers meet all unit type 
