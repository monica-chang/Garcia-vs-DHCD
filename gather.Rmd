...
transfers_include_chars <- function(transfers, chars, hotel = TRUE) {
  ...
  left_join(transfers, chars, by = c("transfer_location" = "name")) %>% 
    ...
    # so I can filter program_to transfers out.
...
# I substitute missing date_acc_met values with transfer information (346 rows).
...
# This only uses the date_acc_met values provided by DHCD (837 requests).
# Only 163/837 are not NA values.
...
# missing date_acc_met values with transfer information (332 requests). 
...
# Checkboxes for:
  # All accs vs. just interesting/unit type accs (scat, ffloor, wheelchair)
  # Substitute missing values w transfers
  # Hotel assumption 
# Display time plot depending on selected boxes
